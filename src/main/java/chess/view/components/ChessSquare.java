...
public class ChessSquare {
    ...
    private Rectangle checkOverlay; // Para indicar jaque
    private Rectangle checkmateOverlay; // Para indicar jaque mate
    private Rectangle lastMoveOverlay; // Para indicar la última jugada
    ...
    private final Color MOVE_HIGHLIGHT_COLOR = Color.rgb(255, 255, 0, 0.4); // Amarillo para última jugada
    private final Color HINT_HIGHLIGHT_COLOR = Color.rgb(0, 255, 255, 0.4);
    private final Color CHECK_HIGHLIGHT_COLOR = Color.rgb(255, 100, 100, 0.6); // Rojo claro para jaque
    private final Color CHECKMATE_HIGHLIGHT_COLOR = Color.rgb(139, 0, 0, 0.7); // Rojo oscuro para jaque mate
    ...
    private void initializeSquare() {
        ...
        // Overlay para indicar jaque (naranja)
        checkOverlay = new Rectangle();
        checkOverlay.widthProperty().bind(root.widthProperty());
        checkOverlay.heightProperty().bind(root.heightProperty());
        checkOverlay.arcWidthProperty().bind(background.arcWidthProperty());
        checkOverlay.arcHeightProperty().bind(background.arcHeightProperty());
        checkOverlay.setFill(CHECK_HIGHLIGHT_COLOR);
        checkOverlay.setVisible(false);

        // Overlay para indicar jaque mate (rojo oscuro)
        checkmateOverlay = new Rectangle();
        checkmateOverlay.widthProperty().bind(root.widthProperty());
        checkmateOverlay.heightProperty().bind(root.heightProperty());
        checkmateOverlay.arcWidthProperty().bind(background.arcWidthProperty());
        checkmateOverlay.arcHeightProperty().bind(background.arcHeightProperty());
        checkmateOverlay.setFill(CHECKMATE_HIGHLIGHT_COLOR);
        checkmateOverlay.setVisible(false);

        // Overlay para indicar la última jugada (amarillo)
        lastMoveOverlay = new Rectangle();
        lastMoveOverlay.widthProperty().bind(root.widthProperty());
        lastMoveOverlay.heightProperty().bind(root.heightProperty());
        lastMoveOverlay.arcWidthProperty().bind(background.arcWidthProperty());
        lastMoveOverlay.arcHeightProperty().bind(background.arcHeightProperty());
        lastMoveOverlay.setFill(MOVE_HIGHLIGHT_COLOR);
        lastMoveOverlay.setVisible(false);
    ...
    private void loadPieceImage(String pieceSymbol) {
        // Mapear símbolo Unicode a nombre de archivo
        String imageName = mapSymbolToImageName(pieceSymbol);
        ...
        Image image = ImageCache.getImage(imageName);

        if (image == null) {
            System.err.println("✗ Imagen no encontrada en cache: " + imageName);
            pieceImageView.setVisible(false);
            pieceImageView.setImage(null);
            return;
        }
    ...
    /**
     * Mapea símbolos Unicode de piezas a nombres de archivos de imagen
     */
    private String mapSymbolToImageName(String symbol) {
        if (symbol == null || symbol.isEmpty()) {
            return null;
        }

        // Mapeo de símbolos Unicode a nombres de imagen
        switch (symbol) {
            // Piezas blancas
            case "♙":
                return "white_pawn";
            ...
            default:
                System.err.println("✗ Símbolo desconocido: " + symbol);
                return null;
        }
    }
    ...
    /**
     * Muestra indicador de jaque en esta casilla (naranja)
     */
    public void showCheckIndicator() {
        if (checkOverlay != null) {
            checkOverlay.setVisible(true);
            FadeTransition fadeIn = new FadeTransition(Duration.millis(300), checkOverlay);
            fadeIn.setFromValue(0.0);
            fadeIn.setToValue(1.0);
            fadeIn.play();
        }
    ...
    /**
     * Oculta indicador de jaque
     */
    public void hideCheckIndicator() {
        if (checkOverlay != null && checkOverlay.isVisible()) {
            FadeTransition fadeOut = new FadeTransition(Duration.millis(300), checkOverlay);
            fadeOut.setFromValue(checkOverlay.getOpacity());
            fadeOut.setToValue(0.0);
            fadeOut.setOnFinished(e -> checkOverlay.setVisible(false));
            fadeOut.play();
        }
    }

    /**
     * Muestra indicador de jaque mate en esta casilla (rojo oscuro)
     */
    public void showCheckmateIndicator() {
        if (checkmateOverlay != null) {
            checkmateOverlay.setVisible(true);
            // Hacer que parpadee
            FadeTransition pulse1 = new FadeTransition(Duration.millis(400), checkmateOverlay);
            pulse1.setFromValue(0.5);
            pulse1.setToValue(1.0);
    ...
    /**
     * Oculta indicador de jaque mate
     */
    public void hideCheckmateIndicator() {
        if (checkmateOverlay != null) {
            checkmateOverlay.setVisible(false);
        }
    }
    ...
    /**
     * Muestra indicador de última jugada (amarillo)
     */
    public void showLastMoveIndicator() {
        if (lastMoveOverlay != null) {
            lastMoveOverlay.setVisible(true);
            FadeTransition fadeIn = new FadeTransition(Duration.millis(300), lastMoveOverlay);
            fadeIn.setFromValue(0.0);
            fadeIn.setToValue(1.0);
            fadeIn.play();
        }
    ...
    /**
     * Oculta indicador de última jugada
     */
    public void hideLastMoveIndicator() {
        if (lastMoveOverlay != null && lastMoveOverlay.isVisible()) {
            FadeTransition fadeOut = new FadeTransition(Duration.millis(300), lastMoveOverlay);
            fadeOut.setFromValue(lastMoveOverlay.getOpacity());
            fadeOut.setToValue(0.0);
            fadeOut.setOnFinished(e -> lastMoveOverlay.setVisible(false));
            fadeOut.play();
        }
    }
}