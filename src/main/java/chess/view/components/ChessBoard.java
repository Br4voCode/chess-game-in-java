...
import chess.model.history.Step;
...
public class ChessBoard {
    ...
    private void initializeBoard() {
        ...
        this.animationManager = new BoardAnimationManager(boardContainer, squares);
     ...
     *
     * <p>
     * This is intended to reduce flicker during undo/redo animations: instead of
     * repainting
     * the full board, we only repaint the minimal set of squares that may have
     * changed.
     */
    ...
    /**
     * Obtiene el gestor de animaciones
     */
    public BoardAnimationManager getAnimationManager() {
        return animationManager;
    }

    /**
     * Muestra indicador de jaque (check) en la casilla del rey
     */
    public void showCheckOnKing(Position kingPosition) {
        ChessSquare kingSquare = getSquare(kingPosition);
        if (kingSquare != null) {
            kingSquare.showCheckIndicator();
        }
    }
     ...
     */
    public void hideCheckIndicators() {
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
                ChessSquare square = squares[row][col];
                if (square != null) {
                    square.hideCheckIndicator();
                }
            }
        }
    }
    ...
    /**
     * Muestra indicador de jaque mate (checkmate) en la casilla del rey
     */
    public void showCheckmateOnKing(Position kingPosition) {
        ChessSquare kingSquare = getSquare(kingPosition);
        if (kingSquare != null) {
            kingSquare.showCheckmateIndicator();
        }
    }
     ...
     */
    public void hideCheckmateIndicators() {
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
                ChessSquare square = squares[row][col];
                if (square != null) {
                    square.hideCheckmateIndicator();
                }
            }
        }
    }
    ...
    /**
     * Muestra indicador de última jugada (amarillo) en dos casillas
     */
    public void showLastMoveIndicators(Position fromPosition, Position toPosition) {
        ChessSquare fromSquare = getSquare(fromPosition);
        ChessSquare toSquare = getSquare(toPosition);
        if (fromSquare != null) {
            fromSquare.showLastMoveIndicator();
        }
        if (toSquare != null) {
            toSquare.showLastMoveIndicator();
        }
    }

    /**
     * Oculta indicadores de última jugada
     */
    public void hideLastMoveIndicators() {
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 8; col++) {
                ChessSquare square = squares[row][col];
                if (square != null) {
                    square.hideLastMoveIndicator();
                }
            }
        }
    }
